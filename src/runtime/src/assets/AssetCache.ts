...
import { IAssetDataOfType } from '@polyzone/runtime/cartridge/data';
import type { IAssetDb, IMaterialAssetData, IMeshAssetData, IMeshSupplementaryAssetData, IScriptAssetData, ISoundAssetData, ITextureAssetData } from '@polyzone/runtime/cartridge/data';
...
export type CachedAssetFactory = () => Promise<LoadedAsset>;
...
export class AssetCache {
  ...
  /**
   * Cache of loaded asset (promises), keyed by asset ID.
   */
  private assetInstanceCache: Map<string, Promise<LoadedAsset>>;
  ...
  public constructor(engine: IEngine, assetDb: IAssetDb) {
    this.engine = engine;
  ...
  public set(assetId: string, assetFactory: (context: AssetCacheContext) => Promise<LoadedAsset>): void {
    ...
    const cachedFactory: CachedAssetFactory = () => {
   ...
   */
  public async loadAsset<TAssetType extends AssetType>(asset: IAssetDataOfType<TAssetType>): Promise<LoadedAssetOfType<TAssetType>> {
    try {
      const cached = this.assetInstanceCache.get(asset.id);
      if (cached) {
        console.log(`[${AssetCache.name}] (${this.loadAsset.name}) Asset already loaded, returning cached asset: ${asset.path}`);
        ...
        if (cachedFactory === undefined) {
          ...
          cachedFactory = () => {
            ...
            console.log(`[${AssetCache.name}] (${this.loadAsset.name}) Loading new instance of asset: ${asset.path}`);
        ...
        const newInstancePromise = cachedFactory() as Promise<LoadedAssetOfType<TAssetType>>;

        // Store new instance in cache
        this.assetInstanceCache.set(asset.id, newInstancePromise);
