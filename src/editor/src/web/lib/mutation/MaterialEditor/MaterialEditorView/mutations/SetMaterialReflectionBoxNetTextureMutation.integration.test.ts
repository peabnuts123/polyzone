...
import { AssetType, ITextureAssetData, MeshAssetMaterialOverrideReflectionBoxNetData, MeshAssetMaterialOverrideReflectionBoxNetDefinition } from '@polyzone/runtime/src/cartridge';
...
import { SetMaterialReflectionBoxNetTextureMutation } from './SetMaterialReflectionBoxNetTextureMutation';

describe(SetMaterialReflectionBoxNetTextureMutation.name, () => {
  test("Setting reflection texture on material with box-net reflection type but no texture updates state correctly", async () => {
    // Setup
    ...
    function getDataValue(): ITextureAssetData | undefined {
      return (mockMaterialEditorViewController.materialData.reflection as MeshAssetMaterialOverrideReflectionBoxNetData)?.texture;
    }
    ...
    function getJsonValue(): string | undefined {
      return (mockMaterialEditorViewController.materialJson.value.reflection as MeshAssetMaterialOverrideReflectionBoxNetDefinition)?.textureAssetId;
    }
    ...
    const mutation = new SetMaterialReflectionBoxNetTextureMutation(mockTextureAssetDefinition.id);
  ...
  test("Changing reflection texture on material with existing box-net texture updates state correctly", async () => {
    // Setup
    ...
    const mutation = new SetMaterialReflectionBoxNetTextureMutation(mockTextureAssetDefinition2.id);
  ...
  test("Removing reflection texture on material with existing box-net texture updates state correctly", async () => {
    // Setup
    let mockTextureAssetDefinition!: TextureAssetDefinition;
    let mockMaterialAssetDefinition!: MaterialAssetDefinition;
    let mockMaterial!: MockMaterial;
    ...
    const mutation = new SetMaterialReflectionBoxNetTextureMutation(undefined);
  ...
  test("Attempting to set reflection texture on material without box-net reflection type throws error", async () => {
    // Setup
    let mockTextureAssetDefinition!: TextureAssetDefinition;
    let mockMaterialAssetDefinition!: MaterialAssetDefinition;
    let mockMaterial!: MockMaterial;
    const mock = new MockProject(({ manifest, asset, scene }) => {
      mockTextureAssetDefinition = asset(AssetType.Texture, 'textures/skybox.png', MockAssets.textures.stonesPng);
      mockMaterial = createMockMaterial({
        // @NOTE No reflection config - material doesn't have box-net reflection type set
      });
    ...
    // Assert
    await expect(testFunc, "Should throw error when trying to set box-net reflection texture on material without box-net reflection type")
  ...
  test("Attempting to set reflection texture on material with different reflection type throws error", async () => {
    ...
    const mock = new MockProject(({ manifest, asset, scene }) => {
      ...
      mockMaterial = createMockMaterial({
        reflection: {
          type: 'separate', // @NOTE Different reflection type
          strength: 1.0,
        },
      });
    ...
    // Assert
    await expect(testFunc, "Should throw error when trying to set box-net reflection texture on material with different reflection type")
      .rejects.toThrow("Cannot set reflection texture for material - the material doesn't have the correct reflection type set");
  });
});
