...
import { SetMaterialReflection3x2TextureMutation } from './SetMaterialReflection3x2TextureMutation';

describe(SetMaterialReflection3x2TextureMutation.name, () => {
  test("Setting reflection texture on material with 3x2 reflection type but no texture updates state correctly", async () => {
    // Setup
    ...
    function getDataValue(): ITextureAssetData | undefined {
      return (mockMaterialEditorViewController.materialData.reflection as MeshAssetMaterialOverrideReflection3x2Data)?.texture;
    }
    ...
    const mutation = new SetMaterialReflection3x2TextureMutation(mockTextureAssetDefinition.id);
  ...
  test("Changing reflection texture on material with existing 3x2 texture updates state correctly", async () => {
    // Setup
    ...
    const mutation = new SetMaterialReflection3x2TextureMutation(mockTextureAssetDefinition2.id);
  ...
  test("Removing reflection texture on material with existing 3x2 texture updates state correctly", async () => {
    // Setup
    let mockTextureAssetDefinition!: TextureAssetDefinition;
    let mockMaterialAssetDefinition!: MaterialAssetDefinition;
    let mockMaterial!: MockMaterial;
    ...
    const mutation = new SetMaterialReflection3x2TextureMutation(undefined);
  ...
  test("Attempting to set reflection texture on material without 3x2 reflection type throws error", async () => {
    // Setup
    let mockTextureAssetDefinition!: TextureAssetDefinition;
    let mockMaterialAssetDefinition!: MaterialAssetDefinition;
    let mockMaterial!: MockMaterial;
    ...
    // Assert
    await expect(testFunc, "Should throw error when trying to set 3x2 reflection texture on material without 3x2 reflection type")
  ...
  test("Attempting to set reflection texture on material with different reflection type throws error", async () => {
    ...
    // Assert
    await expect(testFunc, "Should throw error when trying to set 3x2 reflection texture on material with different reflection type")
      .rejects.toThrow("Cannot set reflection texture for material - the material doesn't have the correct reflection type set");
  });
});
