...
export class SetModelMaterialOverrideReflectionBoxNetTextureMutation implements IModelEditorViewMutation {
  // Mutation parameters
  ...
  public async apply({ ProjectController, ModelEditorViewController }: ModelEditorViewMutationArguments): Promise<void> {
    ...
    if (reflectionTextureAssetData) {
      const reflection = await ReflectionLoading.loadBoxNet(materialOverridesData.reflection, ProjectController.assetCache, ModelEditorViewController.scene);
