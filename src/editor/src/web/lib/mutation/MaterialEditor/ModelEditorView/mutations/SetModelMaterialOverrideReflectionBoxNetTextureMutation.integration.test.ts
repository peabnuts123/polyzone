...
import { SetModelMaterialOverrideReflectionBoxNetTextureMutation } from './SetModelMaterialOverrideReflectionBoxNetTextureMutation';

describe(SetModelMaterialOverrideReflectionBoxNetTextureMutation.name, () => {
  test("Setting reflection texture on model material override with box-net reflection type but no texture updates state correctly", async () => {
    // Setup
    ...
    function getDataValue(): ITextureAssetData | undefined {
      return (mockMeshAssetData.getOverridesForMaterial(mockMaterialName)?.reflection as MeshAssetMaterialOverrideReflectionBoxNetData)?.texture;
    }
    ...
    function getJsonValue(): string | undefined {
      ...
      return (assetDefinition.materialOverrides![mockMaterialName].reflection as MeshAssetMaterialOverrideReflectionBoxNetDefinition)?.textureAssetId;
    }
    ...
    const mutation = new SetModelMaterialOverrideReflectionBoxNetTextureMutation(
  ...
  test("Changing reflection texture on model material override with existing box-net texture updates state correctly", async () => {
    // Setup
    const mockMaterialName = 'Main'; // @NOTE Must match `sphere.mtl` - referenced in mock project definition
    let mockMeshAssetDefinition!: MeshAssetDefinition;
  ...
  test("Removing reflection texture on model material override with existing box-net texture updates state correctly", async () => {
    // Setup
    const mockMaterialName = 'Main'; // @NOTE Must match `sphere.mtl` - referenced in mock project definition
    let mockMeshAssetDefinition!: MeshAssetDefinition;
    let mockTextureAssetDefinition!: TextureAssetDefinition;
  ...
  test("Attempting to set reflection texture on material override without box-net reflection type throws error", async () => {
    // Setup
    const mockMaterialName = 'Main'; // @NOTE Must match `sphere.mtl` - referenced in mock project definition
    let mockMeshAssetDefinition!: MeshAssetDefinition;
    let mockTextureAssetDefinition!: TextureAssetDefinition;
    ...
    // Assert
    await expect(testFunc, "Should throw error when trying to set box-net reflection texture on material override without box-net reflection type")
  ...
  test("Attempting to set reflection texture on material override with different reflection type throws error", async () => {
    ...
    // Assert
    await expect(testFunc, "Should throw error when trying to set box-net reflection texture on material override with different reflection type")
      .rejects.toThrow("Cannot set reflection texture for material override - the material doesn't have a reflection override yet");
  });
});
