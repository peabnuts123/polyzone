...
import { SetModelMaterialOverrideReflection3x2TextureMutation } from './SetModelMaterialOverrideReflection3x2TextureMutation';

describe(SetModelMaterialOverrideReflection3x2TextureMutation.name, () => {
  test("Setting reflection texture on model material override with 3x2 reflection type but no texture updates state correctly", async () => {
    // Setup
    ...
    function getDataValue(): ITextureAssetData | undefined {
      return (mockMeshAssetData.getOverridesForMaterial(mockMaterialName)?.reflection as MeshAssetMaterialOverrideReflection3x2Data)?.texture;
    }
    ...
    function getJsonValue(): string | undefined {
      ...
      return (assetDefinition.materialOverrides![mockMaterialName].reflection as MeshAssetMaterialOverrideReflection3x2Definition)?.textureAssetId;
    }
    ...
    const mutation = new SetModelMaterialOverrideReflection3x2TextureMutation(
  ...
  test("Changing reflection texture on model material override with existing 3x2 texture updates state correctly", async () => {
    // Setup
    const mockMaterialName = 'Main'; // @NOTE Must match `sphere.mtl` - referenced in mock project definition
    let mockMeshAssetDefinition!: MeshAssetDefinition;
  ...
  test("Removing reflection texture on model material override with existing 3x2 texture updates state correctly", async () => {
    // Setup
    const mockMaterialName = 'Main'; // @NOTE Must match `sphere.mtl` - referenced in mock project definition
    let mockMeshAssetDefinition!: MeshAssetDefinition;
    let mockTextureAssetDefinition!: TextureAssetDefinition;
  ...
  test("Attempting to set reflection texture on material override without 3x2 reflection type throws error", async () => {
    // Setup
    const mockMaterialName = 'Main'; // @NOTE Must match `sphere.mtl` - referenced in mock project definition
    let mockMeshAssetDefinition!: MeshAssetDefinition;
    let mockTextureAssetDefinition!: TextureAssetDefinition;
    const mock = new MockProject(({ manifest, asset, scene }) => ({
      ...
      assets: [
        ...
        mockMeshAssetDefinition = asset(AssetType.Mesh, 'models/sphere.obj', MockAssets.models.sphereObj, {
          materialOverrides: {
            [mockMaterialName]: {
              // @NOTE No reflection config - material doesn't have 3x2 reflection type set
            },
          },
        }),
        asset(AssetType.MeshSupplementary, 'models/sphere.mtl', MockAssets.models.sphereMtl),
    ...
    // Assert
    await expect(testFunc, "Should throw error when trying to set 3x2 reflection texture on material override without 3x2 reflection type")
  ...
  test("Attempting to set reflection texture on material override with different reflection type throws error", async () => {
      ...
      assets: [
        ...
        mockMeshAssetDefinition = asset(AssetType.Mesh, 'models/sphere.obj', MockAssets.models.sphereObj, {
          materialOverrides: {
            [mockMaterialName]: {
              reflection: {
                type: '6x1', // @NOTE Different reflection type
                strength: 1.0,
              },
            },
          },
    ...
    // Assert
    await expect(testFunc, "Should throw error when trying to set 3x2 reflection texture on material override with different reflection type")
      .rejects.toThrow("Cannot set reflection texture for material override - the material doesn't have a reflection override yet");
  });
});
